...
```
Sources/Audiosmith/
├── AudiosmithApp.swift               # @main MenuBarExtra entry point
...
| No permission prompt / silent input | Run from `dist/Audiosmith.app`, not `swift run`; check System Settings → Privacy & Security → Microphone |
...
- A fully custom Core Audio Server Plug-In (no BlackHole dependency), which would allow renaming the virtual device to "Audiosmith Input"
